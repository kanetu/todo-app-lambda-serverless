...
// src/todo/postTodo.ts
var postTodo_exports = {};
__export(postTodo_exports, {
  createTodo: () => createTodo
});
module.exports = __toCommonJS(postTodo_exports);
...
var query = {
  ...
  insertTodo: "INSERT INTO todo(task, status, due_date, priority) VALUES($1, $2, $3, $4) RETURNING *",
  deleteTodo: "DELETE FROM todo WHERE id = $1",
  updateTodo: "UPDATE todo SET task = $1, status = $2, priority = $3 WHERE id = $4 RETURNING *"
};

// src/todo/getTodos.ts
var secretName = "rds-db-credentials/cluster-NZ3RT5QDHPTEPD2A2DBAQLT4Z4/admin/1725607992574";

// src/todo/postTodo.ts
var createTodo = async (_event, _context) => {
  console.log("start::createTodo");
  const { task, status, due_date, priority } = JSON.parse(
    _event.body || ""
  );
  ...
  try {
    await db.connect();
    result = await db.query(query.insertTodo, [task, status, due_date, priority]);
  } catch (err) {
